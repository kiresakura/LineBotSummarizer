...
│  Groups  │     │ (FastAPI)│     │(5min/20msg)│    │(OpenRouter)│    │ Writer │
└──────────┘     └──────────┘     └───────────┘     └──────────┘     └────────┘
                                       │
                                  ┌────┴────┐
                                  │ Parser  │
...
│   ├── pipeline/
│   │   ├── parser.py        # 訊息解析、URL 擷取、媒體下載
│   │   ├── aggregator.py    # 滑動視窗聚合
│   │   ├── classifier.py    # AI 多模態分類與摘要
...
2. **下載** — 圖片/音訊訊息從 LINE Content API 下載二進位內容
3. **聚合** — 同群組訊息在 5 分鐘視窗內累積（最少 3 則、最多 20 則）
4. **路由** — 偵測批次中的媒體類型，自動選擇最適合的 AI 模型
5. **分類** — AI 分析文字 + 圖片 + 音訊，回傳分類、重要性、摘要、標籤、待辦事項
