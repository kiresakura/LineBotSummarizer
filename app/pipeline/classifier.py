...
4. **title**：為這批知識點取一個簡潔明確的標題（繁體中文，30字以內）

5. **tags**：提取 3-5 個關鍵詞標籤（繁體中文）

6. **action_items**：如果有待辦事項或需要跟進的事，列出來
...
class MessageClassifier:
    ...
    def _format_url_contents(self, messages: list[RawMessage]) -> str:
        ...
        parts = ["\n\n## 連結內容（已爬取）\n"]
        for i, uc in enumerate(all_contents[:3], 1):
            parts.append(f"### 連結 {i}: {uc['title']}")
            parts.append(f"URL: {uc['url']}")
            parts.append(uc["content"][:3000])
            parts.append("")

        parts.append("請根據以上連結內容進行深入統整，提取核心知識點。")
        return "\n".join(parts)
