...
class NotionWriter:
    ...
    def _build_properties(self, classified: ClassifiedMessage) -> dict:
        ...
        properties = {
            ...
            "Tags": {"multi_select": [{"name": tag} for tag in classified.tags[:5]]},
            "Source Group": {"select": {"name": classified.group_name or "未知群組"}},
            "Date": {
                "date": {
                    "start": classified.original_messages[0].timestamp.isoformat()
                    if classified.original_messages
                    else None
                }
            },
            "Summary": {"rich_text": _split_rich_text(clean_summary)},
            "Has Action Items": {"checkbox": len(classified.action_items) > 0},
        }
        ...
        blocks.append(
            {
                "object": "block",
                "type": "heading_2",
                "heading_2": {"rich_text": [{"text": {"content": "📚 知識點整理"}}]},
            }
        )
            ...
            blocks.append(
                {
                    "object": "block",
                    "type": "heading_2",
                    "heading_2": {"rich_text": [{"text": {"content": "✅ 待辦事項"}}]},
                }
            )
            ...
            blocks.append(
                {
                    "object": "block",
                    "type": "heading_2",
                    "heading_2": {"rich_text": [{"text": {"content": "🔗 相關連結"}}]},
                }
            )
        ...
        blocks.append(
            {
                "object": "block",
                "type": "heading_2",
                "heading_2": {"rich_text": [{"text": {"content": "💬 原始訊息"}}]},
            }
        )
            ...
            blocks.append(
                {
                    "object": "block",
                    "type": "quote",
                    "quote": {"rich_text": _split_rich_text(content)},
                }
            )

        # Notion 單次建立頁面最多 100 個子區塊
        if len(blocks) > 100:
            logger.warning(f"內容區塊 {len(blocks)} 個超過 Notion 上限 100，截斷尾部")
            blocks = blocks[:100]
